...
For example, think about a the coin-flipping experiment where you flip 10 coints. Let $X$ denote the random variable counting the number of heads that land out of the 10 flips. It is important to note $X$ is not associated with any particular value. You can do this experiment many different times and $X$ represents \emph{the process} of running the experiment, not the value of a particular trial. But, we know $X$ will take on a value in the set $\{ 0, 1, 2, \dots, 10 \}$, say $x = 6$ for a particular trial.
...
Lastly, we will discuss transformations of random variables. So For example, say you have a function $g$ that takes the random variable, $X$, of the number of heads out of $10$ and assign to sum final score, $Y$. That is, $Y = g(X)$. The average value of $Y$ can be calculated using the pdf of $X$:
$$
...
In this class, we care about how variables relate to one another. For example, do sales grow and shrink with a consumer's age?; is height an important predictor of basketball success?; is the sale associated with a large increase in sales?
...
See Figure \ref{fig:visual_covariance} for an example of calculating the sample covariance.

The covariance is negative if when $X$ is above it's mean, $Y$ tends to be below it's mean and vice versa. 
They move in opposite directions, but are still related! In other words, if I know $X$ was above it's mean, then I would predict that $Y$ is below it's mean (knowing one gives me information on the other).


\begin{figure}
  \caption{Visual Intuition of Covariance}\label{fig:visual_covariance}
  \begin{center}
    \includegraphics[width=0.8\textwidth]{figures/covariance_visualized.pdf}
  \end{center}
\end{figure}
...
\textbf{Statistical Inference} is the procedure of using a random sample of observations from a population to try and learn something about the population distribution of the data. To be more specific, there is some \textbf{parameter} of the population distribution (e.g. it's expectation, it's variance, the 80th percentile, etc.) that we want to know about. 
Let's call that parameter $\theta$.
If we knew the values of the variables for the entire population, we could simply calculate this parameter. 
For example, average the value of $X_i$ for all the people in your population and you will know it's expected value.
...
We do not observe the full population; only just a sample. 
We take our random sample, $X_1, \dots, X_n$, and want to \textbf{estimate} the population statistic. 
We calculate an \textbf{estimate} of the population parameter using the data. 
The calculation we chose to make is called our \textbf{estimator}. 
For example, we can use the sample average from a set of observations to infer about the expectation of the variable in the population. 
To do so, we could calculate the sample average of our sample:
$$
  \underbrace{\bar{X}}_{\text{Estimator}} \equiv 1/n \sum_{i=1}^n X_i
$$

Any particular way to estimate the population parameter is called a \textbf{estimator}. 

To summarize:
\begin{itemize}
  \item Population Parameter: Function of data in the entire \emph{population}
  
  \item Estimator: Function of data in your sample
\end{itemize}

\subsection{Evaluting Estimators}

For any statistic, there are many different estimators. Therefore, we have some ways to discuss what makes a good estimator.

\noindent\textbf{Property 1: \textbf{Unbiasedness}}
...
Note this is not saying the estimate always equals the population statistic, just that on average it does. For example, the sample mean does not always equal the population mean, but it is an unbiased estimate for the population mean.

\noindent\textbf{Property 2: \textbf{Consistency}}
...
\noindent\textbf{Property 3: \textbf{Efficiency}}
...
It turns out that for a lot of estimators, the sample distribution is approximately \textbf{normally distributed}.
That the sample distribution is approximately normal (in large samples) is proven by a \textbf{central limit theorem}.
It is \emph{really} common for estimators to have a normal sample distribution, but not always.

When the sample distribution is normal, then it is really simple to summarize uncertainty around our estimator: with confidence intervals and hypothesis tests.  

\subsection*{Confidence Intervals}

We typically will construct a \textbf{confidence interval} to descirbe our level of certainty. The confidence interval is centered on our estimate, but it's width describes a range of values we think the population statistic could be.
