...
```{r}
# setwd("03-Regression_Theory/r-day/")
...
You can create this matrix using `diag(resid)`. 
...
cat(paste0("The 95% confidence interval is (", round(ci_lower, 3), ", ", round(ci_upper, 3), ")"))
```
...
cat(paste0("Our predicted wage for a 40 year old worker is ", round(yhat_i, 2), "."))
```
...
```{r}
vcov_yhat_i <- t(W_i) %*% vcov_hat %*% W_i
se_yhat_i <- sqrt(diag(vcov_yhat_i))
cat(paste0("Our predicted wage for a 40 year old worker is ", round(yhat_i, 2), " with standard error ", round(se_yhat_i, 2), "."))
```
...
```{r}
ci_forecast_lower <- yhat_i - 1.96 * se_yhat_i
ci_forecast_upper <- yhat_i + 1.96 * se_yhat_i
cat(paste0("The 95% confidence interval for our forecast is (", round(ci_forecast_lower, 3), ", ", round(ci_forecast_upper, 3), ")"))
```
...
```{r}
cbind(
  coef(est) - 1.96 * se(est), coef(est) + 1.96 * se(est)
)
```
...
```{r}
df_i <- data.frame(age = 40)
predict(est, newdata = df_i)
predict(est, newdata = df_i, se.fit = TRUE)
```
...
```{r}
est <- feols(
  wage ~ 1 + age + age^2 + age^3 + factor(college),
  data = df, vcov = "hc1"
)
print(est)
```
...
```{r}
predict(est, newdata = data.frame(age = 35, college = 1))
```
...
```{r}
predict(est, newdata = data.frame(age = 35, college = 1)) -
  predict(est, newdata = data.frame(age = 30, college = 0))
```
