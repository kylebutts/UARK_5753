...
```{r}
month(football$date, label = TRUE, abbr = FALSE)
```
...
```{r}
table(month(football$date))
table(month(football$date, label = TRUE, abbr = FALSE))
```
...
```{r}
plot(unrate ~ date, unemployment, type = "l")
```
...
```{r}
quarter(unemployment$date, type = "year.quarter")
```
...
```{r}
fourth_of_july <- ymd("2025-07-04")
fourth_of_july - today
```
...
```{r}
wday(fourth_of_july, label = TRUE, abbr = FALSE)
```
...
```{r}
days <- seq(ymd("1990-01-01"), today, by = 1)
days[month(days) == 2 & day(days) == 29]

n_leap_years <- length(days[month(days) == 2 & day(days) == 29])
n_leap_years
```
...
```{r}
days <- seq(ymd("2000-01-01"), today, by = 1)
days[wday(days, label = TRUE) == "Fri" & day(days) == 13]

n_friday_the_13th <- length(days[wday(days, label = TRUE) == "Fri" & day(days) == 13])
n_friday_the_13th
```
